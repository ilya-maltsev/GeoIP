...
def geoip_query(ip_address):
    try:
        ...
        try:
            ...
            print("Please download and gunzip the latest version of GeoIPCity.dat.gz from https://mailfud.org/geoip-legacy/")
            sys.exit()
        ...
        try:
            ...
            print("Please download and gunzip the latest version GeoIPASNum.dat from https://mailfud.org/geoip-legacy/")
            sys.exit()
        ...
        return [ip_address, as_number, as_name, city, region_name, country_name,
                country_code, str(latitude), str(longitude)]
...
def print_csv(ip_data_dict):
    ...
    try:
        for ip, info in ip_data_dict.items():
            if info:
                print(",".join(info))
        exit(1)
    except Exception as e:
